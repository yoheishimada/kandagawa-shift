...
@st.cache_resource
def load_models():
    path = os.path.join(DATA_DIR, "models.pkl")
    try:
...
前月のスプレッドシートが埋まったら、ターミナルで以下を実行してアプリを再起動するだけです（約5分）。
...
| 症状 | 対処 |
|---|---|
| アプリが開かない | MacでアプリとWi-Fiの接続を確認 |
| 天気が取れないと表示される | 17日以上先は天気予報が使えません。予測は続きます |
| 数字が古い気がする | アプリを再起動すると更新されます |
